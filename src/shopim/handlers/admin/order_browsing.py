...
def get_order_search_results_keyboard(
    ...
    builder = InlineKeyboardBuilder()
    for order in orders:
        text = _("№{order_number} - {user_full_name} ({total_amount:.2f} so'm)").format(order_number=order.order_number, user_full_name=order.user.full_name, total_amount=order.total_amount)
        builder.button(
            text=text,
