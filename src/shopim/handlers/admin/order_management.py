    ...
    if order:
        if callback.message:
            await callback.message.edit_text(
                _("✅ Buyurtma (№{order_number}) {admin_full_name} tomonidan tasdiqlandi.").format(order_number=order.order_number, admin_full_name=admin.full_name)
            )
        ...
        if user:
            try:
                await bot.send_message(
                    user.telegram_id,
                    _("🎉 Sizning №{order_number} buyurtmangiz tasdiqlandi va tez orada qadoqlanadi.").format(order_number=order.order_number),
                )
        ...
        if callback.message:
            await callback.message.edit_text(
                _("Buyurtma topilmadi yoki allaqachon ko'rib chiqilgan.")
            )
...
async def reject_order_start_handler(
    ...
    if callback.message:
        await callback.message.answer(
            _("Iltimos, ushbu buyurtmani rad etish sababini yozing.")
        )
    await callback.answer()
