...
async def cancel_creation_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(_("Mahsulot yaratish bekor qilindi."))  # type: ignore
...
async def start_product_creation_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(  # type: ignore
        _("Yangi mahsulot yaratish boshlandi.\n\n"
...
async def get_name_handler(message: types.Message, state: FSMContext, session: AsyncSession):
    if not message.text or len(message.text) < 3:
        await message.answer(_("Mahsulot nomi kamida 3 belgidan iborat bo'lishi kerak."))  # type: ignore
        return
...
async def get_category_handler(
    ...
    await callback.message.edit_text(  # type: ignore
        _("Kategoriya tanlandi. Endi mahsulot uchun batafsil tavsif kiriting."),
...
async def skip_description_handler(callback: types.CallbackQuery, state: FSMContext):
    await state.update_data(description=None)  # type: ignore
    await state.set_state(ProductCreationState.getting_image)  # type: ignore
    await callback.message.edit_text(  # type: ignore
        _("Tavsif o'tkazib yuborildi. Endi mahsulot rasmini yuboring."),
...
async def get_description_handler(message: types.Message, state: FSMContext):
    await state.update_data(description=message.text)  # type: ignore
    await state.set_state(ProductCreationState.getting_image)  # type: ignore
    await message.answer(  # type: ignore
        _("Tavsif qabul qilindi. Endi mahsulot rasmini yuboring."),
        reply_markup=get_skip_or_cancel_keyboard("skip_image"),
    )
...
async def skip_image_handler(callback: types.CallbackQuery, state: FSMContext):
    await state.update_data(image_file_id=None)  # type: ignore
    await state.set_state(ProductCreationState.getting_cost_price)  # type: ignore
    await callback.message.edit_text(  # type: ignore
        _("Rasm o'tkazib yuborildi. Endi mahsulotning 1 gramm uchun tannarxini kiriting (so'mda)."),
        reply_markup=get_cancellation_keyboard(),
    )
...
async def get_image_handler(message: types.Message, state: FSMContext):
    await state.update_data(image_file_id=message.photo[-1].file_id)  # type: ignore
    await state.set_state(ProductCreationState.getting_cost_price)  # type: ignore
    await message.answer(  # type: ignore
        _("Rasm qabul qilindi. Endi mahsulotning 1 gramm uchun tannarxini kiriting (so'mda)."),
        reply_markup=get_cancellation_keyboard(),
    )
...
async def wrong_image_handler(message: types.Message):
    await message.answer(  # type: ignore
        _("Iltimos, rasm yuboring yoki bu qadamni o'tkazib yuboring."),
        reply_markup=get_skip_or_cancel_keyboard("skip_image"),
    )
...
async def get_cost_price_handler(message: types.Message, state: FSMContext):
    ...
    if cost_price is None:
        await message.answer(_("Xato. Tannarxni musbat raqamda kiriting (masalan, 15000.50)."))  # type: ignore
        return

    await state.update_data(cost_price=str(cost_price))
    await state.set_state(ProductCreationState.getting_sale_price)
    await message.answer(  # type: ignore
        _("Tannarx qabul qilindi. Endi 1 gramm uchun sotuv narxini kiriting (so'mda)."),
        reply_markup=get_cancellation_keyboard(),
    )
...
async def get_sale_price_handler(message: types.Message, state: FSMContext):
    ...
    if sale_price is None:
        await message.answer(_("Xato. Sotuv narxini musbat raqamda kiriting (masalan, 20000)."))  # type: ignore
        return

    await state.update_data(sale_price=str(sale_price))
    await state.set_state(ProductCreationState.getting_initial_stock)
    await message.answer(  # type: ignore
        _("Sotuv narxi qabul qilindi. Endi boshlang'ich qoldiqni grammda kiriting (masalan, 1000)."),
        reply_markup=get_cancellation_keyboard(),
    )
...
async def get_initial_stock_handler(message: types.Message, state: FSMContext):
    ...
    if initial_stock is None:
        await message.answer(_("Xato. Boshlang'ich qoldiqni musbat raqamda kiriting (masalan, 1000)."))  # type: ignore
        return
    ...
    await message.answer(  # type: ignore
        _("Boshlang'ich qoldiq qabul qilindi. Endi 'low stock' ogohlantirishi uchun minimal miqdorni kiriting (grammda)."),
        reply_markup=get_cancellation_keyboard(),
    )
...
async def get_low_stock_threshold_handler(
    ...
    if low_stock_threshold is None:
        await message.answer(_("Xato. Minimal miqdorni musbat raqamda kiriting (masalan, 100)."))  # type: ignore
        return
    ...
    try:
        ...
        await message.answer(  # type: ignore
            _("✅ Mahsulot muvaffaqiyatli yaratildi!\n\n"
              "Nomi: <b>{product_name}</b>\n"
              "ID: {product_id}\n"
              "Sotuv narxi: {sale_price:.2f} so'm/gr\n"
              "Qoldiq: {stock_grams} gr").format(
                product_name=new_product.name, product_id=new_product.id,
                sale_price=new_product.sale_price_per_gram, stock_grams=new_product.stock_grams
            ),
            parse_mode="HTML",
        )
    except Exception as e:
        # Log error `e`
        await message.answer(_("❌ Mahsulot yaratishda xatolik yuz berdi: {error_message}").format(error_message=e))  # type: ignore