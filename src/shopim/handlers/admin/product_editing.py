...
async def _show_products_for_editing(
    ...
    if isinstance(target, types.CallbackQuery):
        await target.message.edit_text(text, reply_markup=keyboard)  # type: ignore
    else:
...
async def confirm_delete_product_handler(
    ...
    if deleted:
        ...
        await callback.message.edit_text(  # type: ignore
            _("❌ Mahsulotni o'chirishda xatolik yuz berdi. Balki u buyurtmalar bilan bog'liqdir.")
        )
...
async def _process_decimal_update(
    ...
    try:
        ...
        await message.answer(_("Xato. Iltimos, musbat raqam kiriting."))  # type: ignore
...
async def process_new_low_stock_handler(message: types.Message, state: FSMContext, session: AsyncSession):
    await _process_decimal_update(
        message, state, session, "low_stock_threshold_grams", _("Low stock chegarasi yangilandi")
    )